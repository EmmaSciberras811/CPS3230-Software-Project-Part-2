...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\User\\workspace\\testrv/output_loginRV.txt");
...
int _state_id_LoginAttempt = 201;
...
if (0==1){}
else if (_state_id_LoginAttempt==201){
		if (1==0){}
		else if ((_occurredEvent(_event,386/*inLoginScreen*/)) && (inLogin ==false )){
		_cls_loginRV0.pw .println ("Invalid Login. Back in login screen, try again!");

		_state_id_LoginAttempt = 201;//moving to state LOGGEDOUT
		_goto_LoginAttempt(_info);
		}
		else if ((_occurredEvent(_event,384/*validLogin*/)) && (inLogin ==true )){
		_cls_loginRV0.pw .println ("Valid Login event. @ Home Page");

		_state_id_LoginAttempt = 199;//moving to state LOGGEDIN
		_goto_LoginAttempt(_info);
		}
}
else if (_state_id_LoginAttempt==199){
		if (1==0){}
		else if ((_occurredEvent(_event,388/*inAlertScreen*/)) && (inAlert ==true )){
		_cls_loginRV0.pw .println (" @ Alert Page");

		_state_id_LoginAttempt = 200;//moving to state VIEWALERTS
		_goto_LoginAttempt(_info);
		}
		else if ((_occurredEvent(_event,386/*inLoginScreen*/)) && (inLogin ==true )){
...
else if (_state_id_LoginAttempt==200){
		if (1==0){}
		else if ((_occurredEvent(_event,388/*inAlertScreen*/)) && (inLogin ==true )){
		_cls_loginRV0.pw .println ("Valid Logout");
...
switch(_state_id){
case 201: if (_mode == 0) return "LOGGEDOUT"; else return "LOGGEDOUT";
case 199: if (_mode == 0) return "LOGGEDIN"; else return "LOGGEDIN";
case 200: if (_mode == 0) return "VIEWALERTS"; else return "VIEWALERTS";
